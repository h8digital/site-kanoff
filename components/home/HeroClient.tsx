...
export default function HeroClient({ titulo, subtitulo, bgUrl, cta1, cta2, whatsapp, stats }: Props) {
  const canvasRef = useRef<HTMLCanvasElement>(null)

  // Partículas — só exibe quando não tem imagem de fundo
  useEffect(() => {
    if (bgUrl) return  // se tem imagem, não precisa de partículas
  ...
  return (
    <section style={{
      ...
      alignItems: 'center',
      overflow:   'hidden',
      // Se tem imagem: usa ela. Senão: gradiente escuro
      background: bgUrl
        ? `linear-gradient(rgba(10,5,30,0.55), rgba(10,5,30,0.7)), url('${bgUrl}') center/cover no-repeat`
        : 'linear-gradient(135deg, #0A051E 0%, #12103A 50%, #0A051E 100%)',
    }}>
      {/* Canvas de partículas — só quando não tem imagem */}
      {!bgUrl && (
      ...
      <div className="container" style={{ position:'relative', zIndex:2, paddingTop:120, paddingBottom:80 }}>
        <div style={{ maxWidth:700 }}>
          ...
          <h1 style={{ fontSize:'clamp(36px,5.5vw,72px)', marginBottom:24, lineHeight:1.1 }}
            dangerouslySetInnerHTML={{ __html: titulo.replace(/PRONTOS|READY/g, '<span class="neon-text">$&</span>') }} />

          {/* Fallback se dangerouslySetInnerHTML não funcionar */}
          <style>{`.hero-titulo .neon-text { color: #FFB800; text-shadow: 0 0 5px rgba(255,184,0,0.8), 0 0 10px rgba(255,184,0,0.5), 0 0 20px rgba(255,184,0,0.3); }`}</style>
          ...
          <div style={{ display:'flex', gap:16, flexWrap:'wrap' }}>
            ...
            <a href={`https://wa.me/${whatsapp}?text=Olá! Gostaria de uma cotação.`}
